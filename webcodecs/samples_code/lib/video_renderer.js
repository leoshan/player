...
export class VideoRenderer {
  async initialize(demuxer, canvas) {
    ...
    this.decoderConfig = config; // Store the configuration
  ...
  chooseFrame(timestamp) {
	//console.log(`Looking for frame at timestamp: ${timestamp}`);
  ...
  bufferFrame(frame) {
	//console.log(`Buffering frame with timestamp: ${frame.timestamp}`);
  ...
  seek(seekTime) {
    // Close and discard any buffered frames
    while (this.frameBuffer.length > 0) {
      let frame = this.frameBuffer.shift();
      frame.close();
    }
    this.decoder.reset();  // Reset the decoder
    this.decoder.configure(this.decoderConfig); // Reconfigure the decoder
    this.demuxer.seek(seekTime);  // Seek the demuxer
    this.fillFrameBuffer();  // Refill the frame buffer
  }
}
