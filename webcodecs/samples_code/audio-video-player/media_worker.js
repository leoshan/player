...
self.addEventListener('message', async function(e) {
  ...
  switch (e.data.command) {
    case 'initialize':
	  ...
	  // Get the duration from the demuxer
	  let duration = audioDemuxer.getTotalDuration();
    ...
    case 'pause':
      ...
      break;
	case 'seek':
	  let seekTime = e.data.seekTime;
	  updateMediaTime(seekTime, performance.now() + performance.timeOrigin); // 更新媒体时间为 seekTime
	  audioRenderer.seek(seekTime);
	  videoRenderer.seek(seekTime);
	  break;
